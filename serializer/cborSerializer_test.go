...
var _ = Describe("CBOR Serializer", func() {
	...
	It("Remove should remove fields from the payload", func() {
		...
		serial := serializer.CreateCBORSerializer(log.WithField("unit", "test"))
		...
		Expect(snap.SnapshotMulti("CBOR Remove()", p.Remove("Winter", "name"))).ShouldNot(HaveOccurred())
	})
	...
	It("Bson should return a bson map", func() {
		...
		Expect(snap.SnapshotMulti("CBOR Bson()", bs)).ShouldNot(HaveOccurred())
	})
	...
	It("Add should add fields to payload", func() {
		...
		Expect(snap.SnapshotMulti("CBOR AddMany()", m)).ShouldNot(HaveOccurred())
	})

	It("Should handle each return type", func() {
		logger := log.WithField("serializer", "CBOR")
		serializer := serializer.CreateCBORSerializer(logger)
	...
	It("Should transform Reader into Payload", func() {
		serial := serializer.CreateCBORSerializer(log.WithField("unit", "test"))
		b := []byte{161, 102, 102, 105, 101, 108, 100, 88, 102, 118, 97, 108, 117, 101, 90}
		r := bytes.NewReader(b)
		p := serial.ReaderToPayload(r)
		Expect(p.Get("fieldX").String()).Should(Equal("valueZ"))
	})

	It("ReaderToPayload should return error for invalid cbor", func() {
		serial := serializer.CreateCBORSerializer(log.WithField("unit", "test"))
		r := strings.NewReader(`{ some crazy invalid cbor }`)
		p := serial.ReaderToPayload(r)
		Expect(p.Error()).ShouldNot(BeNil())
		Expect(p.Error().Error()).Should(Equal("cbor unmarshal error"))
	})
})
